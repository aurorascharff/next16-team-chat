...
export function startPPRNavigation(navigatedAt, oldUrl, oldRenderedSearch, oldCacheNode, oldRouterState, newRouteTree, newMetadataVaryPath, freshness, seedData, seedHead, seedDynamicStaleAt, isSamePageNavigation, accumulation, // Instant Navigation Testing API only — restricts segment reads to shell
...
restrictToShell) {
    ...
    const segmentEntry = readSegmentCacheEntryForNavigation(now, tree.varyPath, restrictToShell);
    ...
    if (isPage) {
        ...
        if (metadataVaryPath !== null) {
            const metadataEntry = readSegmentCacheEntryForNavigation(now, metadataVaryPath, restrictToShell);
    ...
    try {
        ...
        if (routeCacheEntry !== null && result.staticStageData !== null) {
            ...
            getStaleAt(now, staticStageResponse.s).then((staleAt)=>{
                ...
                writePrerenderResponseIntoCache(now, FetchStrategy.PPR, staticStageResponse.f, buildId, staticStageResponse.h, staticStageResponse.r ?? null, staleAt, dynamicRequestTree, result.renderedSearch, isResponsePartial);
            }).catch(()=>{
