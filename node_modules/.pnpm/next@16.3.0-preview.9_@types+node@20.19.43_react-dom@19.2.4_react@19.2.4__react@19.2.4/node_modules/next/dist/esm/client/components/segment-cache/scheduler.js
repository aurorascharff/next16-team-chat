                    ...
                    case FetchStrategy.PPR:
                        {
                            ...
                            // branch early.
                            if (// Do any segments have runtime prefetching configured? This is
                            // sent by the server as part of the prefetch hints.
                            tree.prefetchHints & PrefetchHint.SubtreeHasRuntimePrefetch || // Are we in the Shell prefetching phase? The Shell phase is
                            ...
                            task.phase === 1) {
                                const runtimeStrategy = task.phase === 1 ? FetchStrategy.RuntimeShell : FetchStrategy.PPRRuntime;
...
function pingStaticHead(now, task, route) {
    ...
    if (// TODO: Currently static App Shell extraction is not implemented for
    ...
    const segments = {
        tree: route.metadata,
        entry: readOrCreateSegmentCacheEntry(now, FetchStrategy.PPR, route.metadata, task._navigationLockPrefetch ?? null),
        parent: null
    };
...
function pingNewPartOfCacheComponentsTree(now, task, route, tree, parentBundle) {
    ...
    if (tree.prefetchHints & PrefetchHint.HasRuntimePrefetch || task.phase === 1) {
 ...
 */ function pingSegmentBundle(now, task, route, routeKey, tree, segments) {
    ...
    while(node !== null){
        ...
        switch(nodeEntry.status){
            case EntryStatus.Empty:
                upgradeToPendingSegment(nodeEntry, FetchStrategy.PPR, task._navigationLockPrefetch ?? null);
                needsFetch = true;
                break;
            case EntryStatus.Pending:
                if (canNewFetchStrategyProvideMoreContent(nodeEntry.fetchStrategy, FetchStrategy.PPR)) {
                    ...
                    if (revalidatingEntry.status === EntryStatus.Empty) {
                        upgradeToPendingSegment(revalidatingEntry, FetchStrategy.PPR, task._navigationLockPrefetch ?? null);
            ...
            case EntryStatus.Fulfilled:
                {
                    ...
                    if (nodeEntry.isPartial && canNewFetchStrategyProvideMoreContent(nodeEntry.fetchStrategy, FetchStrategy.PPR) || isUpgradeableISRFallbackRetry) {
                        const revalidatingEntry = readOrCreateRevalidatingSegmentEntry(now, FetchStrategy.PPR, nodeTree);
                        if (revalidatingEntry.status === EntryStatus.Empty) {
                            upgradeToPendingSegment(revalidatingEntry, FetchStrategy.PPR, task._navigationLockPrefetch ?? null);
                            node.entry = revalidatingEntry;
    ...
    spawnPrefetchSubtask(fetchSegmentsOnCacheMiss(task, route, routeKey, tree, segments, segmentCount));
}
/**
