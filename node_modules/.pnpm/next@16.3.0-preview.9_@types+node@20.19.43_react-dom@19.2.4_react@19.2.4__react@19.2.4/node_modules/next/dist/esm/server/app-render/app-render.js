...
import { anySegmentHasRuntimePrefetchEnabled, anySegmentHasPartialPrefetchingEnabled, isPageAllowedToBlock, anySegmentNeedsInstantValidationInDev, anySegmentNeedsInstantValidationInBuild, resolveInstantConfigSamplesForPage } from './instant-validation/instant-config';
 ...
 */ async function generateStagedDynamicFlightRenderResultNode(req, ctx, requestStore) {
    ...
    if (experimental.cachedNavigations === 'allow-runtime' || Boolean(renderOpts.partialPrefetching) || await anySegmentHasPartialPrefetchingEnabled(loaderTree)) {
    ...
    !isBypassingCachesInDev(initialRequestStore, workStore)) {
        ...
        if (initialRequestStore.isHmrRefresh === true) {
            ...
            if (prefetchMode === 2) {
                ...
                prefetchStage = await anySegmentHasRuntimePrefetchEnabled(loaderTree) ? RenderStage.Runtime : RenderStage.Static;
            }
        }
        // With Server Components HMR cancellation enabled, a superseded HMR refresh
    ...
    return getTracer().withSpan(renderSpan, async ()=>{
        ...
        try {
            ...
            cacheComponents) {
                ...
                if (cachedNavigations === 'allow-runtime' || Boolean(renderOpts.partialPrefetching) || await anySegmentHasPartialPrefetchingEnabled(tree)) {
                    const prerenderResumeDataCache = createPrerenderResumeDataCache();
                    requestStore.resumeDataCache = prerenderResumeDataCache;
                    const cacheSignal = new CacheSignal();
                    trackPendingModules(cacheSignal);
 ...
 * finished.
 */ function runDevValidationInBackground(prefetchMode, navigationKind, resultPromise, requestStore, validationDebugChannel, cacheSignal, ctx, fallbackRouteParams, prerenderResumeDataCache, getDevRenderDidError, createRequestStore, getPayload, onError, validationGeneration) {
    ...
    void consoleAsyncStorage.run({
        ...
        return runValidationInDev(prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, devRenderDidError, validationAbortSignal);
 ...
 */ async function stagedRenderWithCachesInDev({ prefetchMode, ctx, requestStore, createRequestStore, getPayload, onError, shouldValidate, fallbackRouteParams, getDevRenderDidError, navigationKind, requestAbortSignal }) {
    ...
    try {
        ...
        if (validationGeneration !== undefined) {
            runDevValidationInBackground(prefetchMode, navigationKind, resultPromise, requestStore, validationDebugChannel, cacheSignal, ctx, fallbackRouteParams, prerenderResumeDataCache, getDevRenderDidError, createRequestStore, getPayload, onError, validationGeneration);
            ...
            resultPromise.then(()=>forwardInvalidDynamicUsageError(ctx.workStore.invalidDynamicUsageError, ctx), ()=>{});
        }
        return {
            stream,
            debugChannel
...
async function runValidationInDev(...args) {
    if (process.env.__NEXT_TEST_MODE && process.env.NEXT_TEST_LOG_VALIDATION) {
        const ctx = args[3];
        const requestId = ctx.requestId;
        const url = ctx.url.href;
        ...
        try {
            const validationAbortSignal = args[6];
 ...
 */ async function runValidationInDevImpl(prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, devRenderDidError, validationAbortSignal) {
...
async function validateStaticShell(inputs, ctx, rootParams, fallbackRouteParams, debugChunks, hmrRefreshHash, validationAbortSignal) {
    ...
    const runtimeResult = await validateStagedShell(runtimeChunks, dynamicChunks, debugChunks, runtimeStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHoleInRuntimeShell, validationAbortSignal);
    ...
    const staticResult = await validateStagedShell(staticChunks, dynamicChunks, debugChunks, staticStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, trackDynamicHoleInStaticShell, validationAbortSignal);
...
async function validateInstantConfigInBuildWithSample(outerCtx, sample, allPossibleFallbackRouteParams, prefilledDataCache) {
    ...
    return workAsyncStorage.run(workStore, async ()=>{
        ...
        await warmupClientModulesForStagedValidation('validation-client', accumulatedChunks.dynamicChunks, accumulatedChunks.dynamicChunks, sampleRootParams, fallbackRouteParams, validationCtx, validationSamples, warmupValidationSamplesTracking);
        ...
        return await validateInstantConfigs(prefetchMode, accumulatedChunks, debugChunks, startTime, sampleRootParams, fallbackRouteParams, validationCtx, undefined, validationSamples, false // build has no shared dev render that would surface errors
        );
    });
}
/**
    ...
    try {
       ...
       * */ const checkUnexpectedAbort = ()=>{
            ...
            if (shouldGenerateStaticFlightData(workStore)) {
                ...
                const shellIsPartial = didLinkDataUnblockNewContent ? true : resultIsPartial;
                metadata.segmentData ??= new Map();
                metadata.segmentData.set('/_shell', Buffer.concat(prependIsPartialByteToChunks(collectedChunksByStage.shellStaticChunks, shellIsPartial)));
            }
