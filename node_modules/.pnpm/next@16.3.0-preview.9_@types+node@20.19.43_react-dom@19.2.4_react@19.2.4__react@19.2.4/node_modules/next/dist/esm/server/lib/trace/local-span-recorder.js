...
import { isLocalSpanRecordingEnabled, isRequestInsightsEnabled, recordSpan } from './span-store';
...
export function createLocalSpan({ name, attributes, links, startTime, traceId, spanId, parentSpanId, delegateSpan }) {
...
export function getActiveLocalSpan() {
    return localSpanAsyncStorage == null ? void 0 : localSpanAsyncStorage.getStore();
}
export function isLocalRecordingSpan(span) {
    return span instanceof LocalRecordingSpan;
}
export function withLocalSpan(span, fn) {
...
function getTimestamp(time) {
    ...
    if (typeof time === 'number') {
        ...
        return timeOrigin !== undefined && time < timeOrigin ? timeOrigin + time : time;
    }
    const timeOrigin = getPerformanceTimeOrigin();
    return timeOrigin === undefined ? Date.now() : timeOrigin + performance.now();
}
function getPerformanceTimeOrigin() {
    return typeof performance !== 'undefined' && typeof performance.timeOrigin === 'number' ? performance.timeOrigin : undefined;
}
...
function getCurrentRequestIdentity() {
    try {
        ...
        return {
            requestId: (workStore == null ? void 0 : workStore.requestId) ?? (requestInsightsIdentity == null ? void 0 : requestInsightsIdentity.requestId),
            htmlRequestId: (workStore == null ? void 0 : workStore.htmlRequestId) ?? (requestInsightsIdentity == null ? void 0 : requestInsightsIdentity.htmlRequestId),
