...
import { RSC_HEADER, NEXT_HTML_REQUEST_ID_HEADER, NEXT_REQUEST_ID_HEADER, NEXT_RSC_UNION_QUERY, NEXT_ROUTER_PREFETCH_HEADER, NEXT_ROUTER_SEGMENT_PREFETCH_HEADER, NEXT_URL, NEXT_ROUTER_STATE_TREE_HEADER, NEXT_INSTANT_TEST_COOKIE, NEXT_HMR_REFRESH_HEADER } from '../client/components/app-router-headers';
...
export default class Server {
   ...
   */ this.normalize = (pathname)=>{
    ...
    async handleRequest(req, res, parsedUrl) {
        ...
        const requestId = typeof requestIdHeader === 'string' ? requestIdHeader : nanoid();
        ...
        try {
            ...
            if (useMatchedPathHeader) {
                try {
                    ...
                    if (getRequestMeta(req, 'isNextDataReq') && typeof getRequestMeta(req, 'postponed') === 'string') {
            ...
            if (!getRequestMeta(req, 'hmrRefreshHash')) {
                addRequestMeta(req, 'hmrRefreshHash', this.getServerComponentsHmrRefreshHash());
            }
   ...
   */ async prepare() {
    ...
    async pipeImpl(fn, partialContext) {
        ...
        if (!res.sent) {
            ...
            if (this.dev) {
                res.setHeader('Cache-Control', req.headers[NEXT_HMR_REFRESH_HEADER] === '1' ? 'no-store' : 'no-cache, must-revalidate');
    ...
    async renderToResponseWithComponentsImpl({ req, res, pathname, renderOpts: opts }, { components, query }) {
        ...
        !is404Page) {
            ...
            if (!matchesHash) {
                ...
                const url = new URL(getRequestMeta(req, 'initURL') || req.url || '', 'http://localhost');
            ...
            if (isAppPath && this.nextConfig.cacheComponents) {
                ...
                if ((_pathsResults_prerenderedRoutes = pathsResults.prerenderedRoutes) == null ? void 0 : _pathsResults_prerenderedRoutes.length) {
                    ...
                    if (perUrlFallbackRouteParams && perUrlFallbackRouteParams.length > 0) {
                        addRequestMeta(req, 'fallbackParams', createOpaqueFallbackRouteParams(perUrlFallbackRouteParams));
                    }
                }
            }
        }
