...
import { PHASE_PRODUCTION_SERVER, PHASE_DEVELOPMENT_SERVER, REQUEST_INSIGHTS_DEV_ENDPOINT, UNDERSCORE_NOT_FOUND_ROUTE } from '../../shared/lib/constants';
...
import { getRequestInsightsSnapshot, isRequestInsightsEnabled } from './trace/request-insights';
    ...
    const requestHandlerImpl = async (req, res)=>{
        ...
        if (opts.dev && req.url) {
            ...
            if (pathname === REQUEST_INSIGHTS_DEV_ENDPOINT) {
                if (development && blockCrossSiteDEV(req, res, development.config.allowedDevOrigins, opts.hostname)) {
                    return;
                }
                res.setHeader('Content-Type', 'application/json; charset=utf-8');
                if (!config.experimental.requestInsights && !isRequestInsightsEnabled()) {
                    res.statusCode = 404;
