...
function createServerParamsForRoute(underlyingParams, varyParamsAccumulator = null) {
    ...
    if (workUnitStore) {
        switch(workUnitStore.type){
            ...
            case 'prerender-runtime':
                {
                    throw Object.defineProperty(new _invarianterror.InvariantError('createServerParamsForRoute should not be called in runtime prerenders.'), "__NEXT_ERROR_CODE", {
                        value: "E1367",
                        enumerable: false,
                        configurable: true
                    });
        ...
        case 'prerender':
            {
                ...
                if (varyParamsAccumulator !== null) {
                    userspaceParams = (0, _varyparams.createVaryingParams)(varyParamsAccumulator, underlyingParams, optionalCatchAllParamName);
                }
                if ((0, _paramsutils.isEmptyParams)(underlyingParams)) {
                    // This route has no params.
                    return makeUntrackedParams(userspaceParams);
                }
                const fallbackParams = prerenderStore.fallbackRouteParams;
                if ((0, _paramsutils.hasFallbackRouteParams)(underlyingParams, fallbackParams)) {
                    // This params object has one or more fallback params, so we need
                ...
                if (stagedRendering) {
                    ...
                    if (!(0, _paramsutils.allParamsAreRootParams)(underlyingParams, prerenderStore.rootParams)) {
                        const staticParamsStage = _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.staticLinkData;
...
function createRuntimePrerenderParams(underlyingParams, optionalCatchAllParamName, workStore, workUnitStore, varyParamsAccumulator) {
    ...
    // allowed in shells.
    if ((0, _paramsutils.allParamsAreRootParams)(underlyingParams, workUnitStore.rootParams)) {
        return makeUntrackedParams(userspaceParams);
    }
    ...
    const staticParamsStage = _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.runtimeLinkData;
    return stagedRendering.delayUntilStage(staticParamsStage, 'params', userspaceParams);
}
...
function createStagedRenderParamsImpl(workUnitStore, stagedRendering, asyncApiPromises, /** The actual param values, without any instrumentation */ underlyingParams, /** The params object to return to userspace, possibly wrapped in a proxy */ userspaceParams) {
    ...
    if ((0, _paramsutils.hasFallbackRouteParams)(underlyingParams, workUnitStore.fallbackParams)) {
...
function createParamsPromiseFromTrigger(trigger, userspaceParams) {
    if (process.env.NODE_ENV === 'development') {
        ...
        return (0, _dynamicrenderingutils.makePromiseFromTrigger)(trigger, userspaceParams);
    }
}
function noop() {}
...
function createRenderParamsInDev(underlyingParams, userpaceParams, fallbackParams, workStore, requestStore) {
    return makeDynamicallyTrackedParamsWithDevWarnings(underlyingParams, userpaceParams, (0, _paramsutils.hasFallbackRouteParams)(underlyingParams, fallbackParams), workStore, requestStore);
}
...
function makeDynamicallyTrackedParamsWithDevWarnings(underlyingParams, userspaceParams, hasFallbackParams, workStore, requestStore) {
    ...
    // supports copying with spread and we don't want to unnecessarily
    // instrument the promise with spreadable properties of ReactPromise.
    const promise = hasFallbackParams ? (0, _dynamicrenderingutils.makeDevtoolsIOAwarePromise)(userspaceParams, requestStore, _dynamicrenderingutils.RENDER_STAGES_BY_DATA_KIND.runtimeLinkData) : Promise.resolve(userspaceParams);
