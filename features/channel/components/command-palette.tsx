...
function WorkspaceSearchResults({
  ...
  const [channels, messages] = useSuspenseQueries({
    queries: [
      channelSearchQueryOptions(query),
      workspaceSearchMessagesQueryOptions(query),
    ],
  })
  const [activeIndex, setActiveIndex] = useState(0)
  const channelResults: Result[] = channels.data.map((channel) => {
    return { type: 'channel', ...channel }
  })
  const messageResults: Result[] = messages.data.map((message) => {
    return {
