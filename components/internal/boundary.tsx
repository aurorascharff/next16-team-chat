...
export function useBoundaryMode() {
  ...
  if (!ctx)
    throw new Error('useBoundaryMode must be used within BoundaryProvider')
  return ctx
}
...
export function Boundary({ children, label, asChild }: Props) {
  ...
  if (
    ...
    (asChild || typeof children.type === 'string')
  ) {
    return cloneElement(
      children as React.ReactElement<{ 'data-component'?: string }>,
      { 'data-component': name },
    )
  }
